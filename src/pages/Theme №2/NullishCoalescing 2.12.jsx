...
export default function NullishCoalescing() {
	return (
		<Container className='text-light mt-5 mb-5'>
			...
			<CodeNote js={`result = (a !== null && a !== undefined) ? a : b;`} />
			<p>
			...
			<p>
				Например, здесь мы отобразим <Badge bg='secondary'>user</Badge>, если её
				значение не <Badge bg='secondary'>null</Badge>/
...
alert(firstName ?? lastName ?? nickName ?? "Аноним"); // Суперкодер`}
			...
			<p>
				...
				того же, что и <Badge bg='secondary'>??</Badge>, как это было показано в
				предыдущей главе.
			</p>
			<p>
				Например, если в приведённом выше коде заменить{' '}
				<Badge bg='secondary'>??</Badge> на <Badge bg='secondary'>||</Badge>, то
...
alert(firstName || lastName || nickName || "Аноним"); // Суперкодер`}
			...
			<p>
				...
				<Badge bg='secondary'>||</Badge> появился первым. Он существует с самого
				начала в JavaScript, поэтому разработчики долгое время использовали его
				для таких целей.
			</p>
			<p>
				С другой стороны, сравнительно недавно в язык был добавлен оператор
				нулевого слияния <Badge bg='secondary'>??</Badge> – как раз потому, что
				многие были недовольны оператором <Badge bg='secondary'>||</Badge>.
			</p>
...
alert(height ?? 100); // 0`}
			...
			<ul>
				...
				<li>
					<Badge bg='secondary'>height ?? 100</Badge> проверяет, что переменная{' '}
					<Badge bg='secondary'>height</Badge> содержит{' '}
					<Badge bg='secondary'>null</Badge>/
...
let area = height ?? (100 * width) ?? 50;`}
			...
			<p>
				...
				с целью избежать программные ошибки, когда люди начнут переходить с{' '}
...
alert(x); // 2`}
			...
			<p>
				...
				<Badge bg='secondary'>undefined</Badge>, и если это так — присвоить этой
...
alert(userAge) // по-прежнему 18`}
			...
			<p>
				Оператор нулевого слияния <Badge bg='secondary'>??</Badge> — это быстрый
...
height = height ?? 100;`}
			...
			<p>
				...
				<Badge bg='secondary'>&&</Badge> без явно указанного приоритета, то есть
				без скобок.
			</p>
			<p>
