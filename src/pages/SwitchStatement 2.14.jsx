...
export default function SwitchStatement() {
	return (
		<Container className='text-light mt-5 mb-5'>
			...
			<p>
				Конструкция <Badge bg='secondary'>switch</Badge> заменяет собой сразу несколько{' '}
  ...
  default:
    ...
    [break]
			...
			<ul>
				...
				<li>
					Если ни один <Badge bg='secondary'>case</Badge> не совпал – выполняется
					(если есть) вариант <Badge bg='secondary'>default</Badge>.
				</li>
			</ul>
  ...
  default:
    alert( "Нет таких значений" );
			...
			<p>
				...
				<Badge bg='secondary'>4</Badge>. Совпадение найдено, будет выполнен этот
				вариант, со строки <Badge bg='secondary'>alert( 'В точку!' )</Badge> и
				далее, до ближайшего <Badge bg='secondary'>break</Badge>, который прервёт
				выполнение.
			</p>
			...
			>
				...
				<p>
					Если <Badge bg='secondary'>break</Badge> нет, то выполнение пойдёт ниже
					по следующим <Badge bg='secondary'>case</Badge>, при этом остальные
					проверки игнорируются.
				</p>
			</div>

			<p>Пример без <Badge bg='secondary'>break</Badge>:</p>
			<CodeJs
				js={`let a = 2 + 2;
...
switch (a) {
  ...
  default:
    alert( "Нет таких значений" );
			...
			/>
			<p>В примере выше последовательно выполнятся три <Badge bg='secondary'>alert</Badge>:</p>
			<CodeJs
				js={`alert( 'В точку!' );
alert( 'Перебор' );
alert( "Нет таких значений" );`}
			...
			<p>
				И <Badge bg='secondary'>switch</Badge> и <Badge bg='secondary'>case</Badge>{' '}
  ...
  default:
    alert("Это не выполнится");
			...
			<p>
				...
				<Badge bg='secondary'>b + 1</Badge> в <Badge bg='secondary'>case</Badge>, и
				следовательно, код в этом блоке будет выполнен.
			</p>

			<h2>Группировка «case»</h2>
			<p>
				Несколько вариантов <Badge bg='secondary'>case</Badge>, использующих один
				код, можно группировать.
			</p>
			<p>
				Для примера, выполним один и тот же код для <Badge bg='secondary'>case 3</Badge>{' '}
				и <Badge bg='secondary'>case 5</Badge>, сгруппировав их:
			</p>
			<CodeJs
				js={`let a = 3;
...
switch (a) {
  ...
  default:
    alert('Результат выглядит странновато. Честно.');
			...
			<p>
				Возможность группировать <Badge bg='secondary'>case</Badge> – это побочный
				эффект того, как <Badge bg='secondary'>switch/case</Badge> работает без{' '}
				<Badge bg='secondary'>break</Badge>. Здесь выполнение{' '}
			...
			<p>
				Нужно отметить, что проверка на равенство всегда строгая. Значения должны
				быть одного типа, чтобы выполнялось равенство.
			</p>
...
switch (arg) {
  ...
  default:
    alert( 'Неизвестное значение' );
			...
			<ol>
				<li>
					Для <Badge bg='secondary'>'0'</Badge> и <Badge bg='secondary'>'1'</Badge>{' '}
					выполнится первый <Badge bg='secondary'>alert</Badge>.
				</li>
				<li>
				...
				<li>
					...
					<Badge bg='secondary'>3</Badge>. Таким образом, мы имеем «мёртвый код» в{' '}
					<Badge bg='secondary'>case 3</Badge>! Выполнится вариант{' '}
					<Badge bg='secondary'>default</Badge>.
				</li>
			</ol>
			...
			<p>
				Конструкция <Badge bg='secondary'>switch</Badge> представляет собой более
				наглядную альтернативу множественным проверкам{' '}
				<Badge bg='secondary'>if</Badge>.
			</p>
			<p>
			...
			<p>
				Без <Badge bg='secondary'>break</Badge> выполнение продолжается в следующих{' '}
				<Badge bg='secondary'>case</Badge>, что позволяет группировать варианты.
			</p>
			<p>
